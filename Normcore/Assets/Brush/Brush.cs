...
public class Brush : MonoBehaviour
{
    ...
    private void Update()
    {
         var envelope = GetComponent<SimpleAttackDecay>();
        ...
        bool handIsTracking = UpdatePose(node, ref _handPosition, ref _handRotation, ref _handAcc);
        ...
        if (triggerPressed)
        {
            _activeBrushStroke.MoveBrushTipToPoint(_handPosition, _handRotation);
             audiofile.pitch = Mathf.Abs(_handPosition.x % 3);
            audiofile.volume = Mathf.Abs(_handPosition.y % 1);
            if (!isAudioPlaying)
            {
                // audiofile.Play();
                envelope.PlayAudio(500, audiofile);
                isAudioPlaying = true;
            }
        ...
        if (!triggerPressed && _activeBrushStroke != null)
        {
            ...
            {
                // audiofile.Stop();
                envelope.StopAudio(500, audiofile);
                isAudioPlaying = false;
            }

            //audiofile.Stop();
        }
        //if (triggerPressed && !isAudioPlaying)
        //{
        //    isAudioPlaying = true;
        //    audiofile.Play();
        //}
        //else //if (!triggerPressed && isAudioPlaying)
        //{
        //    isAudioPlaying = false;
        //    audiofile.Stop();
        //}
    ...
    private static bool UpdatePose(XRNode node, ref Vector3 position, ref Quaternion rotation, ref Vector3 acc)
    {
        ...
        foreach (XRNodeState nodeState in nodeStates)
        {
            if (nodeState.nodeType == node)
            {
                ...
                bool gotAcc = nodeState.TryGetAcceleration(out nodeAcc);
